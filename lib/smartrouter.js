...
module.exports = new JS.Class(EventEmitter, {
  ...
  start: function (config) {
    ...
    self.amqp.on('error', function (err) {
      var err_msg = util.format('Error while connecting to RabbitMQ: %s. Will try to reconnect in %s', err, reconnect_delay);
      logger.error(err_msg);
      self._storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.ERROR, EVENTS_CONTROLLER_SUBSYSTEM, err_msg));
      //Emit an error on the socket for the client.
      self.emit('amqpError', new Error(err_msg));
    });
        ...
        self.config.endpoints.forEach(function (endpoint) {
          if (endpoint.name === newEndpoint.name) {
            ...
            if (endpoint.sub.indexOf(newEndpoint.id) < 0) {
              ...
              var warn_msg = util.format('Endpoint [/%s/%s] already exists. Aborted.', newEndpoint.name, newEndpoint.id);
              logger.warn(warn_msg);
              self._storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.WARNING, EVENTS_CONTROLLER_SUBSYSTEM, warn_msg));
              socket.emit('endpointRegistered', new Error(warn_msg));
            }
          }
        });
        if (!endpointFound) {
          var err_msg = util.format('[%s] is not a valid Endpoint.', newEndpoint.name);
          logger.error(err_msg);
          self._storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.ERROR, EVENTS_CONTROLLER_SUBSYSTEM, err_msg));
          socket.emit('endpointRegistrationError', new Error(err_msg));
        }
      });
    });
  },
  ...
  check: function (socket, type, message, callback) {
    ...
    socket.get('actorid', function (err, id) {
      if (!id) {
        ...
        var err_msg = util.format('A unidentified actor tried to post the following message on Socket %s : Type=\'%s\' Message=%s',
  ...
  publish: function (destactorid, type, message, socket) {
    ...
    .on('error', function(err) {
      var err_msg = util.format('The queue [%s] does not exist (%s). The following message (type=\'%s\') *is LOST*: %s',
                                destactorid, err, type, JSON.stringify(message));
      logger.error(err_msg);
      self._storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.ERROR, EVENTS_CONTROLLER_SUBSYSTEM, err_msg));
      if (socket) // Prevent new config deployment issues
      {
        socket.emit('error', 'Sorry, your message could not be delivered. Please contact support team.');
      }
      ...
      if (consumerCount < 1)
      {
        var warn_msg = util.format('Queue [%s] does not have any consumers. (Pending message(s) before publishing: %d)', queueName, messageCount);
        logger.warn(warn_msg);
        self._storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.WARNING, EVENTS_CONTROLLER_SUBSYSTEM, warn_msg));
      }
      else
