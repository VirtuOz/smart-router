...
var nodePorts = { 'amqp': 5681, 'amqp-backup': 5680 };
...
var SmartRouter = new JS.Class(EventEmitter, {
  ...
  start: function (config, options) {
    config.amqp.port = nodePorts['amqp'];

    logger.info('Starting SmartRouter... Will listen on port ' + config.amqp.port);
    logger.info('RabbitMQ url is set to ' + config.amqp.url);

    var self = this;
    self.config = config;

    self.amqp = Amqp.createConnection(config.amqp, options);
    self.amqp.on('error', function (err) {
      logger.error('Error connecting to RabbitMQ: ' + err);
      self.amqp.end();

      // switch ports
      self.amqp.options.port = self.amqp.options.port == nodePorts['amqp-backup'] ? nodePorts['amqp'] : nodePorts['amqp-backup'];
      logger.info('Will now try connecting on port: ' + self.amqp.options.port);
      self.amqp.reconnect();
    });
    ...
    self.config.endpoints.forEach(function (endpoint) {
      ...
      if (endpoint.ids.length === 0) {
        ...
        endpoint.ids.forEach(function (id) {
          ...
          logger.info('Endpoint registered on [' + '/' + endpoint.name + '/' + id + ']');
