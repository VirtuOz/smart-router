...
module.exports = new JS.Class(EventEmitter, {
   ...
   * It allows processes to register new endpoints ids in the smart-router.
   ...
   *
   * Note that it is only possible to register new ids for and existing endpoint,
      ...
      socket.on('registerNewEndpointId', function(newEndpoint) {
        ...
        self.config.endpoints.forEach(function (endpoint) {
          if (endpoint.name === newEndpoint.name) {
            endpointFound = true;
            if (endpoint.ids.indexOf(newEndpoint.id) < 0) {
              ...
              endpoint.ids.push(newEndpoint.id); // Adding the new ID to the existing configuration.
