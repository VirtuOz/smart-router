...
module.exports = new JS.Class(EventEmitter, {
  ...
  start: function (config) {
    ...
    self.amqp.on('error', function (err) {
      logger.error('Error while connecting to RabbitMQ: ' + err + '. Will try to reconnect in ' + reconnect_delay);
    });
    self.amqp.on('ready', function() {
      if (self.started) {
        logger.info('Reconnected to RabbitMQ at ' + config.amqp.url);
        return; // We already started the socket.io server, returning.
      }

      logger.info('Connected to RabbitMQ at ' + config.amqp.url + '. Starting Socket.IO server...');
      if (CONFIG.https.use) {
        var options = {
  ...
  listenForNewEndpoints: function () {
    ...
    self.io.sockets.on('connection', function (socket) {
      socket.on('registerNewEndpointId', function(newEndpoint) {
        ...
        self.config.endpoints.forEach(function (endpoint) {
          if (endpoint.name === newEndpoint.name) {
            ...
            if (endpoint.sub.indexOf(newEndpoint.id) < 0) {
              ...
              var warn_msg = 'Endpoint [/' + newEndpoint.name + '/' + newEndpoint.id + '] already exists. Aborted.';
        ...
        if (!endpointFound) {
          var err_msg = '[' + newEndpoint.name + '] is not a valid Endpoint.';
    ...
    socketep.on('connection', function (socket) {
      ...
      self.config.routes.forEach( function (route) {
        if (route.endpoint === '*' || route.endpoint === endpointname) {
          logger.debug('Registering route [' + route.messagetype + '] for endpoint [' + route.endpoint +
                       ']. Socket=' + socket.id);
  ...
  queueSubscribe: function (queuename, socket) {
    logger.info('Subscribing to queue. QueueName=' + queuename + '; Socket='+socket.id);
    var self = this;
      ...
      if (queueData) {
        queueData.forEach(function (item) {
          item.queue.unsubscribe(item.ctag).addCallback(function () {
            logger.info('Unsubscribed from Queue [' + item.queue.name + '] for socket ' + socket.id +
                        ' ; consumerTag=' + item.ctag);
          });
        });
      }
    });
  ...
  publish: function (destactorid, type, message) {
    ...
    logger.debug('Publishing \'' + type + '\' on Queue [' + destactorid + ']');
