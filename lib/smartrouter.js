...
module.exports = new JS.Class(EventEmitter, {
   ...
   *
   */
  queueSubscribe: function (actorid, queuename, socket) {
    ...
    self.amqp.queue(queuename, { autoDelete: false, closeChannelOnUnsubscribe: true,
      ...
      .addCallback(function (ok) {
        socket.set('actorid', actorid, function () {
          logger.info(util.format('Queue created and actor [%s] registered. Handshaking.', actorid));
          socket.emit('hello', {});
        });
  ...
  publish: function (destactorid, type, message, socket) {
    ...
    if (typeof destactorid !== 'string') {
      logger.error("Trying to publish on a bad queue: " + JSON.stringify(destactorid));
      if (socket) // Prevent new config deployment issues
      {
        socket.emit('error', "Trying to publish on a bad queue: " + JSON.stringify(destactorid));
      }
      return;
    }
