        ...
        self.config.endpoints.forEach(function (endpoint) {
          if (endpoint.name === newEndpoint.name) {
            ...
            if (endpoint.sub.indexOf(newEndpoint.id) < 0) {
              ...
              _storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.WARNING, EVENTS_CONTROLLER_SUBSYSTEM, warn_msg));
            }
          }
        });
        if (!endpointFound) {
          ...
          _storeNagiosEvent(new NagiosCheckResponse(NagiosCheckResponseCodes.ERROR, EVENTS_CONTROLLER_SUBSYSTEM, err_msg));
        }
      });
    });
  },
